...
if st.button("Show Decision Tree Diagram") :
    ...
    tree.plot_tree(clf, feature_names=dta.feature_names, class_names=dta.target_names, filled=True, ax=ax)
    buf = io.BytesIO()
    plt.savefig(buf, format="png")
    buf.seek(0)
    st.images(buf, caption="Decision Tree Visualization", use_column_width=True)
